...
comm_elem :: [[Int]] -> [Int]
comm_elem [] = []
comm_elem ls = sort ( [x \\ x <- ls!!0 | all (isMember x) ls && x rem 2 == 1])
...
volume (idx1,idx2) ls = min (ls!!idx1) (ls!!idx2) * (idx2-idx1)

largest_volume :: [Int] -> (Int, Int)
largest_volume ls = [(idx1,idx2)\\idx1<-[0..end], idx2<-[0..end] | volume (idx1,idx2) ls == maxVolume]!!0
where end = (length ls) - 1
      maxVolume = maxList [volume (idx1,idx2) ls\\ idx1 <- [0..end], idx2 <- [0..end]]
